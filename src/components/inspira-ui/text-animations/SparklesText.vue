...
interface Sparkle {
  id: string // 闪光粒子的唯一标识
  x: string // 闪光粒子的水平位置（百分比）
  y: string // 闪光粒子的垂直位置（百分比）
  color: string // 闪光粒子的颜色
  delay: number // 动画延迟时间（秒）
  scale: number // 闪光粒子的缩放比例
  lifespan: number // 闪光粒子的生命周期
}

interface Props {
  text: string // 要显示的文字
  sparklesCount?: number // 闪光粒子数量
  colors?: {
    first: string // 第一种闪光颜色
    second: string // 第二种闪光颜色
  }
