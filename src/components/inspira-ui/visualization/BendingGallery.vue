...
interface CircularGalleryProps {
  items?: { image: string; text: string }[] // 图片数据，不传则使用内置默认图
  bend?: number // 弯曲程度，数值越大弧线越弯
  textColor?: string // 卡片标题文字颜色
  borderRadius?: number // 卡片圆角大小 (0~1)
  font?: string // 标题字体 CSS
  autoScrollSpeed?: number // 自动流动速度，0 为关闭，默认 0.5
}

const props = withDefaults(defineProps<CircularGalleryProps>(), {
  bend: 1, // 默认弯曲度 值越大弧度越大
  textColor: '#ffffff', // 默认文字颜色
  borderRadius: 0.05, // 默认圆角
  font: '500 16px "Noto Sans SC", "PingFang SC", "Microsoft YaHei", sans-serif', // 默认字体
  autoScrollSpeed: 0.003, // 默认自动流动速度，0 为关闭
})
    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
          ...
          void main() {
            vUv = uv;
            vec3 p = position;
            // 顶点波浪效果：用 sin/cos 让图片表面产生水波扭曲，注释掉以关闭
...
class App {
  ...
  onWheel(e: Event) {
    ...
    this.scroll.target += we.deltaY > 0 ? 2 : -2
  ...
  addEventListeners() {
    ...
    // 拖拽中 mousemove/mouseup 仍需 window 监听，防止移出容器后丢失状态
...
watch(
  [() => props.bend, () => props.textColor, () => props.borderRadius, () => props.font, () => props.autoScrollSpeed],
  () => {
