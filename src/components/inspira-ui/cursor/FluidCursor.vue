...
interface Props {
  simResolution?: number // 模拟网格的分辨率，影响流体细节与性能
  dyeResolution?: number // 流体染色（颜色）纹理的分辨率
  captureResolution?: number // 捕获输入事件的分辨率
  densityDissipation?: number // 颜色密度随时间消散的速度
  velocityDissipation?: number // 速度随时间衰减的程度，影响动量保持
  pressure?: number // 流体物理计算中的压力系数
  pressureIterations?: number // 压力求解的迭代次数
  curl?: number // 漩涡/旋度效果的强度，用于增强旋转运动
  splatRadius?: number // 指针在流体中的溅射半径
  splatForce?: number // 指针作用于流体的力
  shading?: boolean // 是否启用流体颜色的明暗效果
  colorUpdateSpeed?: number // 指针颜色动态更新的速度
  backColor?: ColorRGB // 流体背景色，RGB 格式
  transparent?: boolean // 画布背景是否透明
  class?: HTMLAttributes['class'] // 外层容器 div 的额外 CSS 类
}
