...
interface Props {
  color1?: string // 霓虹边框的主色
  color2?: string // 霓虹边框的辅色
  animationType?: 'none' | 'half' | 'full' // 应用于边框的动画类型
  duration?: number // 动画持续时间（秒）
  class?: HTMLAttributes['class'] // 用于样式的额外 CSS 类
}
...
<template>
  <div
    ...
    :class="cn(`relative z-10 inline-block h-10 w-full max-w-sm overflow-hidden rounded-lg p-px`, props.class)">
    <div :class="cn(`neon-border-one rounded-lg`, animationType !== 'none' ? 'animate-neon-border' : '')" />
    <div :class="cn(`neon-border-two rounded-lg`, animationType !== 'none' ? 'animate-neon-border' : '')" />
    <slot />
  </div>
</template>
